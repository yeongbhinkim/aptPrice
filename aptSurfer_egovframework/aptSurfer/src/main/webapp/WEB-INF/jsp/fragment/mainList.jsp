...
<main>
  ...
  <div class="Apt-list-wrap">
    <div class="Apt-list-box">
      <ul class="Apts-box">
        <li class="Apt-each" c:forEach items ="${list} var="item"">
...
<%--               <canvas th:id="${'chart-' + item.apt_id + ' canvas'}" width="240" height="170"></canvas> --%>
              <canvas c:id="${'chart-' + item.apt_id + ' canvas'}" width="240" height="170"></canvas>
...
<%--                    th:text="${item.dan_gi_myeong}+${item.layer}+층">아파트이름</a> --%>
                   ...
                   <a href="<c:url value="/MyHomePrice/detail/${item.apt_id}"/>"
                  	  text="<c:out value="${item.dan_gi_myeong}+${item.layer}+층"/>" >아파트이름</a>
...
<%--                     <span class="mainHashtag" th:text="${item.construction_date}">건축년도</span> --%>
                    ...
                    <span class="mainHashtag" text="<c:out value="${item.city}"/>">위치</span>
                    <br>
                    <span class="mainHashtag" text="<c:out value="${item.square_meter}"/>">전용면적(㎡)</span>
                    <span class="mainHashtag" text="<c:out value="${item.amount}"/>">거래금액(만원)</span>
                    <span class="mainHashtag" text="<c:out value="${item.construction_date}"/>">건축년도</span>
                  </div>
                </li>
                <br>
              </ul>
